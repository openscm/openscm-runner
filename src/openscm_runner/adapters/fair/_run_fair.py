...
def run_fair(cfgs, output_vars):
    ...
    for cfg in cfgs:
        ...
        tempres = ScmDataFrame(data, columns={
            "scenario": scenario,
            "model": model,
            "region": ["unspecified"],
...
def _process_output(fair_output, output_vars):
    ...
    data['Effective Radiative Forcing|Greenhouse Gases'] = np.sum(F[:,:31], axis=1) # does the MAGICC definition include ozone and H2O from CH4 oxidation?
    data['Effective Radiative Forcing|Greenhouse Gases|Kyoto Gases'] = np.sum(F[:,:15], axis=1)
